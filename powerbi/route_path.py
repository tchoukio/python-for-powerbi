...
# origin = "6056 rue portelance Laval"
# destination = "756 rue dandurand Laval"
...
api_url = f"{endpoint}wp.0={origin}&wp.1={destination}&optmz=distance&routeAttributes={attribut}&key={api_key}"

df = Dataframe()
api = Api(api_key)
rsData = api.getDataRoupePath(origin, destination)

dfRoutePath = df.getDataset(rsData.getRoutePath())
dfLabel = df.getLabelDataframe(rsData)
